...
try{
    ...
    if(content){
        ...
        if(prevUserContent){
            const prevContent: Note[] = JSON.parse(prevUserContent);
            const newContent: Note = {content , id:note_id};
            const updatedContent: Note[] = [...prevContent, newContent];
            finalContent = JSON.stringify(updatedContent);
        }else{
            finalContent = JSON.stringify([{content , id:note_id}])
        }
        await redisClient.set(redisID , finalContent)
